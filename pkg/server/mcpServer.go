...
import (
	...
	"github.com/istio-ecosystem/consul-mcp/pkg/serviceregistry"
	"github.com/istio-ecosystem/consul-mcp/pkg/serviceregistry/consul"
...
func (s *ConsulMcpServer) Start() error {
	log.Info("Start MCP server")

	log.Infof("Watch Consul at %s", s.consulAddress)
	if err := s.watchRegistry(s.stopchan); err != nil {
		log.Fatala(err)
		return err
	}
...
func (s *ConsulMcpServer) watchRegistry(stop <-chan struct{}) error {
	var err error
